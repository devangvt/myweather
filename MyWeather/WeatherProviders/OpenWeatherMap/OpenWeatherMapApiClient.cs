...
using DevangsWeather.Extensions;
...
namespace DevangsWeather.OpenWeatherMap
{
    public class OpenWeatherMapApiClient : IOpenWeatherMapApiClient
    {
        ...
        public async Task<OpenWeatherMapForcastResponse> GetForcastByCity(string city, string countryCode=null, int numberOfDays = 7)
        {
            if (string.IsNullOrWhiteSpace(city))
            {
                throw new ArgumentNullException(nameof(city));
            ...
            var url = ApplyOptionsToQuery(GetForcastByCityUrl(city, countryCode, numberOfDays));
            return await GetWeatherForcastAsync(url);
        ...
        private static async Task<OpenWeatherMapForcastResponse> GetWeatherForcastAsync(string url)
        {
            try
            {
                var message = await new HttpClient().GetAsync(url).ConfigureAwait(false);
                ...
                return JsonConvert.DeserializeObject<OpenWeatherMapForcastResponse>(await message.Content.ReadAsStringAsync().ConfigureAwait(false));
        ...
        {
            var url = $"{_options.BaseUrl}/weather?q={city}";
        ...
        private string GetForcastByCityUrl(string city, string countryCode, int numberOfDays)
        {
            var url = $"{_options.BaseUrl}/forecast/daily?q={city}";
            ...
            url = $"{url}&cnt={numberOfDays}";
            return url;
        }
        private string GetWeatherByCityIdUrl(int cityId)
        {
            var url = $"{_options.BaseUrl}/weather?id={cityId}";
            return url;
        }

        private string GetWeatherByCityCoordinatesUrl(float latitude, float longitude)
        {
            var url = $"{_options.BaseUrl}/weather?lat={latitude.Invariant()}&lon={longitude.Invariant()}";
            return url;
        }

        private string GetWeatherByZipCodeUrl(int zipCode, string countryCode)
        {
            var url = $"{_options.BaseUrl}/weather?zip={zipCode},{countryCode}";
            return url;
        }
